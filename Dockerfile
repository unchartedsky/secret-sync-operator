...
ENV GOPATH /go
ADD . /go/src/github.com/unchartedsky/secret-sync-operator
RUN apt update \
    && apt install -y curl
...
COPY --from=0 /go/src/github.com/unchartedsky/secret-sync-operator/build/_output/bin/secret-sync-operator ${OPERATOR}
COPY --from=0 /go/src/github.com/unchartedsky/secret-sync-operator/build/bin /usr/local/bin
